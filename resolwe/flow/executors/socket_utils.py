...
def send_data(s: socket.SocketType, data: dict, size_bytes: int = 5):
...
class Message(Generic[MessageDataType]):
    ...
    def is_valid(message_dict: Dict) -> bool:
        ...
        try:
            ...
            # print("IS VALID: examining", message_dict)
            ...
            #     print(message_dict[key], value_type)
            #     print(isinstance(message_dict[key], value_type))
...
class BaseCommunicator:
    ...
    async def _receive_message(self) -> Optional[Tuple[PeerIdentity, Message]]:
        ...
        result = None
        try:
            self.logger.debug(f"Communicator {self.name} waiting for message.")
            ...
            if receive_task in done:
                ...
                self.logger.debug(
                    f"Communicator {self.name} _receive_message: terminating flag is set, returning None"
                )
        except:
            self.logger.exception(
                f"Communicator {self.name}: exception in _receive_message."
            )
        finally:
    ...
    async def __aenter__(self):
        ...
        self.logger.debug(f"Communicator {self.name}: entering context.")
        return self
    ...
    def _message_status(self, message_uuid: str) -> MessageStatus:
        ...
        self.logger.debug(f"Checking for message status: {message_uuid}")
        self.logger.debug(f"Known messages: {self._uuids_received}")
    ...
    async def send_response(
        ...
        """Send response."""
        self.logger.debug(f"Communicator {self.name}: sending response {response}")
    ...
    async def send_command(
        ...
        self.logger.debug(f"Communicator {self.name}: sending command {command}")
    ...
    async def stop_listening(self):
        ...
        """
        self.logger.debug(f"Communicator {self.name} stopped listening")
        self._terminating.set()
...
class BaseProtocol:
    ...
    async def communicate(self):
                ...
                while not self._should_stop.is_set():
                    ...
                    if has_message_future in done:
                        ...
                        try:
                            ...
                            await self.process_command(peer_identity, received_command)
                    elif communicator_terminating_future in done:
                        # Communicator is closed. Stop.
                        self.logger.debug(
                            f"Communicator {self.communicator.name} stoped"
                        )
        ...
        self.logger.debug(
            f"Communicator {self.communicator.name} stopped communicating"
        )
