...
FULL_TEXT_SEARCH_RANK = "full_text_search_rank"
...
class TextFilterMixin:
    ...
    def filter_text(self, queryset: QuerySet, name: str, value: str):
        """Filter the queryset using full text search.

        Also annotate the queryset with the rank of the search to be used in the
        ordering filter.
        """
        query = SearchQuery(value, config="simple_unaccent")
        # This assumes that field is already a TextSearch vector and thus doesn't need
        # to be transformed. To achieve that F function is required.
        return queryset.filter(**{name: query}).annotate(
            **{FULL_TEXT_SEARCH_RANK: SearchRank(F(name), query)}
        )
...
class OrderingFilter(DrfOrderingFilter):
    ...
    def _remove_invalid_annotation_field(self, queryset, field, view, request):
        ...
        """
        if re.match(r"-?annotations__(?P<field_id>\d+)", field) and issubclass(
            queryset.model, Entity
        ):
