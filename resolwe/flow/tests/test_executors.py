...
class ManagerRunProcessTest(ProcessTestCase):
    ...
    @with_docker_executor
    @tag_process("test-save-number")
    @override_settings(FLOW_MANAGER_KEEP_DATA=True)
    def test_executor_keep_data_true(self):
        ...
        json_file = Path(data.location.get_path(filename="jsonout.txt"))
        stdout_file = Path(data.location.get_path(filename="stdout.txt"))

        self.assertTrue(json_file.exists())
        self.assertTrue(stdout_file.exists())

        # Remove stdout and json files to run the process again.
        json_file.unlink()
        stdout_file.unlink()

        # Create the temporary file that should NOT be purged since keep data
        # is set to True.
        tmp_file = Path(
            data.location.get_path(filename="temporary_file_do_not_purge.txt")
        )
        ...
        self.assertEqual(data.status, Data.STATUS_DONE)
        self.assertEqual(data.process_error, [])
        # Make sure that the temporary file was not deleted.
        self.assertTrue(tmp_file.exists())

    @with_docker_executor
    @tag_process("test-save-number")
    @override_settings(FLOW_MANAGER_KEEP_DATA=False)
    def test_executor_keep_data_false(self):
        # Run the process for the first time.
        data = self.run_process("test-save-number", {"number": 42})
        ...
        # Create the temporary file that should be purged since keep data
        # is set to False.
        tmp_file = Path(
            data.location.get_path(filename="temporary_file_do_not_purge.txt")
        )
        ...
        self.assertEqual(data.status, Data.STATUS_DONE)
        self.assertEqual(data.process_error, [])
        # Make sure that the temporary file was deleted.
        self.assertFalse(tmp_file.exists())
