...
class ProcessOrderingTest(APITestCase):
    ...
    def test_ordering_version(self):
        pass
        # TODO: this test makes no sense. Only the latest version is returned by the
        # default qset anyway.

        # response = self.client.get(self.url, {"ordering": "version"}, format="json")
        # print("Got response", response, response.data)
        # print("Expected first", self.proc_1.__dict__)
        # print("Expected second", self.proc_2.__dict__)
