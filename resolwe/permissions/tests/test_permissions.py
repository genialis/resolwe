...
class CollectionPermissionsTestOldSyntax(ResolweAPITestCase):
    def setUp(self):
    ...
    def test_protect_owner(self):
        ...
        self.collection.set_permission(Permission.OWNER, self.user1)
        # ``owner`` permission cannot be assigned to a group
        ...
        self.assertEqual(
            self.collection.get_permissions(self.user2),
            [Permission.VIEW, Permission.EDIT, Permission.SHARE],
        )
    ...
    def test_nonexisting_user_group(self):
        ...
        self.collection.set_permission(Permission.VIEW, self.user1)
        self.collection.set_permission(Permission.VIEW, self.group)
        ...
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(resp.data["detail"], "Unknown user: 999")
        self.assertEqual(self.collection.get_permissions(self.user1), [Permission.VIEW])
...
class DescriptorSchemaPermissionsTestOldSyntax(ResolweAPITestCase):
    def setUp(self):
        self.group = Group.objects.create(name="Test group")
