...
class Permission(IntEnum):
    ...
    @staticmethod
    def from_name(permission_name: Optional[str]) -> Optional["Permission"]:
        """Get the permission from permission name.

        :returns: Permission object when permission_name is the name of the
            permission, None if permission_name is None and highest permission
            if permission_name is 'ALL'.
...
class PermissionGroup(models.Model):
    ...
    @transaction.atomic
    def set_permission(
        self, permission: Optional[Permission], user_or_group: UserOrGroup
    ):
        ...
        self.permissions.filter(**{entity_name: entity}).delete()
        ...
        if permission is not None:
            ...
            PermissionModel.objects.create(**arguments)

    def get_permission(self, user_or_group: UserOrGroup) -> Optional[Permission]:
        ...
        permission_count = permission_models.count()
        assert permission_count <= 1, (
            f"Too many permissions on permission group {self.pk} for entity "
            f"'{entity_name}' with primary key {entity.pk}."
        )
        if permission_count:
            return permission_models[0].permission
        else:
            return None
...
class PermissionObject(models.Model):
    ...
    def has_permission(self, permission: Optional[Permission], user: User):
